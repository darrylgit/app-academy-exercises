...
def most_frequent_bigram(str)
    ...
    bigrams.keys.inject() do |acc, bigram|
        if !acc
            bigram
        end

        bigrams[acc] < bigrams[bigram] ? bigram : acc
    end
end
...
class Array
    ...
    def pair_sum_count(num)
        # Lol jeez. Again, promise I didn't copy this one, Alvin's just rubbing off on me apparently
        count = 0
